...
contract ActionMintChi is GelatoActionsStandard {
    ...
    function action(address _recipient, uint256 _chiAmount)
        public
        virtual
        delegatecallOnly("ActionMintChi.action")
    {
        try chiToken.mint(_chiAmount) {
        } catch Error(string memory error) {
            revert(string(abi.encodePacked("ActionMintChi.action.mint:", error)));
        } catch {
            revert("ActionMintChi.action.mint: unknown error");
        }

        try chiToken.transfer(_recipient, _chiAmount) returns (bool success) {
            require(success, "ActionMintChi.action.transfer: unsuccessful");
        } catch Error(string memory error) {
            revert(string(abi.encodePacked("ActionMintChi.action.transfer:", error)));
        } catch {
            revert("ActionMintChi.action.transfer: unknown error");
        }
    }
        ...
        returns(string memory)
    {
        if (this.action.selector != GelatoBytes.calldataSliceSelector(_actionData))
            return "ActionMintChi: invalid action selector";
        ...
        if (_chiAmount > 140 || _chiAmount == 0)
            return "ActionMintChi: invalid chi amount";
